...
$edit = $dbh->prepare('SELECT * FROM memo WHERE id = :id');
$edit->execute([':id' => $_GET['id']]);
$edit2 = $edit->fetchAll();
...
if (!empty($_POST['submit'])) {
    if (!empty(trim($_POST['content']))){
        ...
        $ref = $dbh->prepare('UPDATE memo SET content = :content, modification_date = CURRENT_TIME WHERE id = :id');
        $ref->execute([
